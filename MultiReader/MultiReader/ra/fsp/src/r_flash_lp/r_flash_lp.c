...
/* マルチリーダプロジェクト デバッグオプション */
#include "..\..\..\src\DebugOption.h"
...
// 書き込み中の電源断評価開始（ドライバの書込処理途中で強制リセット）
static bool _isDebugStart_ForDflashDrvDebugForcedStop = false;
void DebugStart_ForDflashDrvDebugForcedStop()
{
    _isDebugStart_ForDflashDrvDebugForcedStop = true;
}
#endif
...
#ifdef DFLASH_DRV_DEBUG_FORCED_STOP  // 書き込み中の電源断評価用（ドライバの書込処理途中で強制リセット）
    {
        if(_isDebugStart_ForDflashDrvDebugForcedStop)
        {
            while(1){}  // 無限ループ
        }
    }
