...
// UART0 Rs485 DE Control Callbask Function ( in Task/EventSender.cpp )
extern void CallbackUart0_Rs485DeControl(uint8_t isDeOn);
...
fsp_err_t R_SCI_UART_Write (uart_ctrl_t * const p_api_ctrl, uint8_t const * const p_src, uint32_t const bytes)
{
    ...
    // UART0 RS485 DE Callback ON
    if(p_ctrl->p_cfg->channel == 0)
    {
        CallbackUart0_Rs485DeControl(1);
    }
...
static void r_sci_negate_de_pin (sci_uart_instance_ctrl_t const * const p_ctrl)
{
    // UART0 RS485 DE Callback OFF
    if(p_ctrl->p_cfg->channel == 0)
    {
        CallbackUart0_Rs485DeControl(0);
    }
