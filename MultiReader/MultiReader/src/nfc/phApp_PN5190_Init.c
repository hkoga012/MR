...
//2023.10.24 add DEBUG_PRINT_RAW_DATA
//#define DEBUG_PRINT_RAW_DATA
extern void phApp_CallBack(void);
extern phStatus_t phApp_Pn5190_SPIRead(uint8_t * pRxBuffer, uint16_t wRxLength);
...
void phApp_CallBack(void)
{
    ...
    if ((dwEventStatusReg & PH_PN5190_EVT_RFON_DETECT) != 0U)
    {
        ...
        bSPIReadLen = (uint8_t)(pHal->sIrqResp.bIsrBytesRead -
...
/*!------------------------------------------------
@brief アンテナ出力電圧設定
@note電圧設定(0x00-0x2Aの範囲とする)
     設定電圧は1.5V + Voltage/10で計算
     例：0x1Fの場合1.5V+(0x1F/10) = 4.6V
     電流設定(0-350の範囲とする デフォルトは306)
@param
@return　2:エラー、0:更新無し、1:更新成功
@author
-------------------------------------------------*/
#define PN5190_EEPROM_TXLDO_VDDPA_MAX_RDR_ADDR  0x08
uint8_t phApp_SetOutputPowerVoltage(uint8_t Voltage)
{
    phStatus_t wStatus;
    uint8_t ReadVoltage = 0;

    if( Voltage > NFC_OUTPUT_VOLTAGE_MAX  )
    {
        //範囲外
        return 2;
    }
    ...
    wStatus = phhalHw_Pn5190_Instr_ReadE2Prom(
        pHal,
        PN5190_EEPROM_TXLDO_VDDPA_MAX_RDR_ADDR,
        &ReadVoltage,
        1U
        );
    ...
    //設定値と読出し値を比較し、異なる場合は設定値を書込み
    if ( Voltage != ReadVoltage)
    {
        wStatus = phhalHw_Pn5190_Instr_WriteE2Prom(
            pHal,
            PN5190_EEPROM_TXLDO_VDDPA_MAX_RDR_ADDR,
...
/*!------------------------------------------------
@brief アンテナ出力電流設定
@note 電流設定(0-350の範囲とする デフォルトは306)
@param
@return　2:エラー、0:更新無し、1:更新成功
@author
-------------------------------------------------*/
#define PN5190_EEPROM_TARGET_CURRENT_ADDR  0x77
uint8_t phApp_SetOutputPowerCurrent(uint16_t Current)
{
    phStatus_t wStatus;
    uint16_t ReadCurrent = 0;

    if( Current > NFC_OUTPUT_CURRENT_MAX  )
    {
        //範囲外
        return 2;
    }
    ...
    wStatus = phhalHw_Pn5190_Instr_ReadE2Prom(
        pHal,
        PN5190_EEPROM_TARGET_CURRENT_ADDR,
        (uint8_t*)&ReadCurrent,
        2U
        );
    if (wStatus != PH_ERR_SUCCESS)
...
uint16_t phApp_GetPn5910FwVersion()
{
    phhalHw_Pn5190_Version_t v;
    v.wFW_Version = 0xffff;
    phhalHw_Pn5190_Instr_GetVersion(pHal, &v);
    return v.wFW_Version;
}
