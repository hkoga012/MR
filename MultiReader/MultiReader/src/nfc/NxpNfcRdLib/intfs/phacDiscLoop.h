 ...
 *
 * \ref phacDiscLoop_Sw_DataParams_t::wId is initialized with this ID in the
 ...
 * */
#ifndef PHAC_DISCLOOP_CFG_MAX_CARDS_SUPPORTED

#define PHAC_DISCLOOP_CFG_MAX_CARDS_SUPPORTED                 0x03U   //2023.12.06 複数検出を可能とするため変更 0x01U -> 0x03U

#endif /* PHAC_DISCLOOP_CFG_MAX_CARDS_SUPPORTED */
 ...
 * will increase if this value is increased.
 * */
#ifndef PHAC_DISCLOOP_CFG_MAX_VICINITY_CARDS_SUPPORTED

#define PHAC_DISCLOOP_CFG_MAX_VICINITY_CARDS_SUPPORTED        0x01U

#endif /* PHAC_DISCLOOP_CFG_MAX_VICINITY_CARDS_SUPPORTED */
 ...
 * - Type A (As per NFC Activity 2.2 and EMVCo Specification 3.1)\n
 * - Type B (As per NFC Activity 2.2 and EMVCo Specification 3.1)\n
 * - Type F @ 212 kbps (As per NFC Activity 2.2)\n
 * - Type F @ 424 kbps (As per NFC Activity 2.2)\n
 * - Type V (ISO 15693) (As per NFC Activity 2.2)\n
...
/**
* \name Supported NFC Activity version.
* \brief Discovery loop supports NFC Activity version 2.2.
*/
/*@{*/
#define PHAC_DISCLOOP_NFC_ACTIVITY_VERSION_2_2                0x22U    /**< Supports NFC Activity version 2.2. */
/*@}*/
...
typedef struct
{
    ...
    struct /*phacDiscLoop_TypeF_Tag_Info*/
    {
        ...
        uint8_t bSleepAFState;                                                              /**< SLEEP_AF State. */
        uint8_t bLength;                                                                    /**< Length of PollingResponse to know if RD bytes are present */
...
typedef struct
{
    ...
    uint8_t bMode;                                                    /**< Refers to the mechanism to specify in a request the set of VICC�s that shall answer to the request. \n
                                                                           values can be #PHPAL_SLI15693_FLAG_SELECTED, #PHPAL_SLI15693_FLAG_ADDRESSED and #PHPAL_SLI15693_FLAG_NON_ADDRESSED. \n
                                                                           default #PHPAL_SLI15693_FLAG_ADDRESSED */
...
typedef struct
{
     ...
     *
     * #RD_LIB_MODE_EMVCO -> Run in EMVCO mode as per the EMVCo 3.1 Digital specification.\n
     * #RD_LIB_MODE_NFC   -> Run in NFC mode as per NFC Forum Activity Specification 2.2. This is the default mode.\n
     *
    ...
    /**
     * Supported NFC Activity version 2.2.
     *
     * #PHAC_DISCLOOP_NFC_ACTIVITY_VERSION_2_2 -> NFC Forum Activity 2.2 mode.\n
     *
     * Use \ref phacDiscLoop_GetConfig with #PHAC_DISCLOOP_CONFIG_ACTIVITY_VERSION
     * option to read above mode.
     * */
     ...
     * */
    uint8_t * pPasTechPollSeq;

    /**
     * Maximum supported Frame Size Card Integer.
     *
     * Application can configure this Parameter before EMVCo Polling loop execution if different Fsci is required other than default.
     * Note: This Parameter is used only in EMVCo Mode. Valid values: 8 and 10 based on Platform.
     * */
    uint8_t bFsciMax;

    /**
     * Flag to indicate whether Optional RF Field Reset is enabled or not for Proprietary technologies polling in EMVCo Polling mode .
     *
     * #PHAC_DISCLOOP_POS_BIT_MASK_F212      -> Optional RF Field Reset Type FeliCa @ 212 kbps.\n
     * #PHAC_DISCLOOP_POS_BIT_MASK_F424      -> Optional RF Field Reset Type FeliCa @ 424 kbps.\n
     * #PHAC_DISCLOOP_POS_BIT_MASK_V         -> Optional RF Field Reset Type V / ISO 15693.\n
     * #PHAC_DISCLOOP_POS_BIT_MASK_18000P3M3 -> Optional RF Field Reset ISO 18000p3m3/ EPC Gen2.\n
     *
     * Use \ref phacDiscLoop_SetConfig with #PHAC_DISCLOOP_CONFIG_EMVCO_PROP_OP_RF_RESET
     * option to set Optional Field Reset enable for Proprietary technologies in EMVCo Polling.
     *
     * Use \ref phacDiscLoop_GetConfig with #PHAC_DISCLOOP_CONFIG_EMVCO_PROP_OP_RF_RESET
     * option to get Optional RF Field Reset enable status for Proprietary technologies in EMVCo Polling.
     *
     * Note: This configuration is applicable for only above mentioned technologies and only in EMVCo Mode.
     * */

    uint8_t bEMVCoPropOpRfFieldReset;
...
*  \brief Discovery loop component as per NFC Forum Activity Specification 2.2 and EMVCo Digital 3.1 specification.
*
*  Discovery loop implements poll mode based on NFC Activity Specification 2.2 and EMVCo Specification 3.1.
*  Listen mode is implemented based on NFC Activity Specification 2.2.
*  ISO 18000-3 Mode 3 (EPC Gen2) is supported as a proprietary technology in poll mode.
*
*<b>Poll_Mode</b>
...
/**
 * Configure the mechanism to specify in a request the set of VICC痴 that shall answer to the request
 *
 * #PHPAL_SLI15693_FLAG_ADDRESSED\n
 * #PHPAL_SLI15693_FLAG_SELECTED\n
 *
 * Default is #PHPAL_SLI15693_FLAG_ADDRESSED.
 * */
#define PHAC_DISCLOOP_CONFIG_TYPEV_MODE                       0x61U
...
/**
 * Read NFC activity version.
 *
 * #PHAC_DISCLOOP_NFC_ACTIVITY_VERSION_2_2 -> Activity 2.2 mode.\n
 * */
...
/**
 * Configure Frame Size Card Integer.
 *
 * This configuration is used to configure maximum allowed Fsci value if different Fsci is required other than default.
 * Note: This configuration is applicable only in EMVCo Mode. Valid values: 8 and 10 based on Platform.
 * */
#define PHAC_DISCLOOP_CONFIG_EMVCO_PROF_FSCI_MAX              0x91U

/**
 * Set/Get Flags to indicate whether Optional RF Field Reset is enabled or not for Proprietary technologies polling in EMVCo Polling mode.
 *
 * #PHAC_DISCLOOP_POS_BIT_MASK_F212      -> Optional RF Field Reset Type FeliCa @ 212 kbps.\n
 * #PHAC_DISCLOOP_POS_BIT_MASK_F424      -> Optional RF Field Reset Type FeliCa @ 424 kbps.\n
 * #PHAC_DISCLOOP_POS_BIT_MASK_V         -> Optional RF Field Reset Type V / ISO 15693.\n
 * #PHAC_DISCLOOP_POS_BIT_MASK_18000P3M3 -> Optional RF Field Reset ISO 18000p3m3/ EPC Gen2.\n
 *
 * This configuration is used to Set/Get Optional RF Field Reset in EMVCo Polling mode for Proprietary technologies polling.
 * Note: This configuration is applicable for only above mentioned technologies and only in EMVCo Mode.
 * */
#define PHAC_DISCLOOP_CONFIG_EMVCO_PROP_OP_RF_RESET           0x92U

/**
 * Configure Sleep state of a Type A device/tag.
 *
 * Lower byte indicates the Sleep state to be either enable/disable.
 * Upper byte indicates the index of Type A device.
 * */
#define PHAC_DISCLOOP_CONFIG_TYPEA_SLEEP_STATE                0x93U

/**
 * Configure Sleep state of a Type B device/tag.
 *
 * Lower byte indicates the Sleep state to be either enable/disable.
 * Upper byte indicates the index of Type B device.
 * */
#define PHAC_DISCLOOP_CONFIG_TYPEB_SLEEP_STATE                0x94U
/*@}*/
 ...
 *
 * \Note: Polling technology sequence shall be with in the defined technology bit map as per \ref DiscLoop_TechConfig.\n
...
phStatus_t phacDiscLoop_CfgPollSeq(
    void * pDataParams,                /**< [In] Pointer to discovery loop parameter structure. */
    uint8_t * pDiscloopCfgPollSeq      /**< [In] Discovery loop polling loop sequence pointer. */
    );
/** @} */
...
//2023.11.09 add
extern uint8_t pFeliCaSystemCode[2];
